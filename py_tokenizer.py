...
def printtoken(type, token, srow_scol, erow_ecol, line): # for testing
    ...
    # name==def ? def ON
    # is def? -> put dict : not && check indent
    # is name==function ? -> print function
    '''
    if(defStart == True):
        if(functionName != ""):
            if(functionName in functions):
                functions[functionName] += '#'+tok_name[type]
            else:
                functions[functionName] = '#'+tok_name[type]
            if(type == 5):         # INDENT
                indentCnt += 1
                ...
                indentCnt -= 1
                if(indentCnt == 0): # end function
                    defStart = False
                    functionName = ""
        else:
            functionName = token           
    elif(type == 1 and token == 'def'): # is def?
        defStart = True
    '''
    print('#'+tok_name[type]+','+str(srow), end="")
